<template>
  <q-page class="">
    <div class="tw-flex tw-gap-8 tw-my-8 tw-px-8 tw-justify-between">
      <div class="tw-flex-1">
        <q-btn
    ...
    <section class="2xl:tw-w-3/5 tw-mx-auto">
      <div
        class="-tw-mt-16 tw-relative tw-h-[500px] tw-overflow-hidden 2xl:tw-rounded-3xl tw-w-full tw-mx-auto tw-shadow-2xl tw-shadow-black"
      >
      ...
      <div
        class="tw-mt-16 tw-mx-4 tw-bg-black/50 tw-backdrop-blur-sm tw-rounded-xl tw-ring-white/20 tw-ring-[2px] tw-p-8 tw-pt-12 tw-font-mono tw-text-lg tw-relative tw-transition-all tw-duration-500 tw-ease-out-expo tw-min-h-[12rem] tw-flex tw-flex-col"
      >
        <div
          class="tw-absolute tw-top-0 tw-bottom-0 tw-left-0 tw-right-0 tw-rounded-xl tw-overflow-hidden tw-opacity-30"
        >
          <img
        ...
        <div v-for="(line, i) in term.lines.value" :key="line.content">
          <div v-if="line.type === 'prompt'" class="tw-my-4">
        ...
        <div
          class="tw-left-0 tw-right-0 tw-bottom-0 tw-flex tw-flex-wrap tw-gap-2 tw-max-h-20"
        >
          <q-btn
            flat
            no-caps
          ...
          <q-btn
            ...
            @click="rmRf"
            :loading="termBusy"
            >/rm -rf --no-preserve-root</q-btn
          >
        </div>

        <!-- <pre class="tw-absolute tw-bottom-0 tw-left-0 tw-p-8">
...
onMounted(async () => {
  while (true) {
    ...
    const texts = [
      ...
      'chmod 777 *',
      'rm -rf /',
      'use the commands on the right to ask questions!',
    ];
...
setInterval(() => {
  let twiceAsFastT =
    (getPlaybackTime() % (60 / (87 * (2 / 3)))) / (60 / (87 * (2 / 3)));
